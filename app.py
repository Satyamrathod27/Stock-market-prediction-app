...
with st.sidebar:
    st.markdown("# Stock Analysis & Prediction")
    user_input = st.selectbox(
    'Please select the stock for forecasting and technical analysis ',
    ('TCS.NS','CAREERP.NS','APOLLOHOSP.NS','NESTLEIND.NS','SAREGAMA.NS'))
    # user_input = st.text_input('Enter Stock Name', "TCS.NS")
    st.markdown("### Choose Date for your anaylsis")
    date_from = st.date_input("From",datetime.date(2020, 3, 6))
    date_to = st.date_input("To",datetime.date(2023, 3, 6))
    options = st.selectbox(
    'Select a stock for diversification analysis',
    ['TCS.NS', 'CAREERP.NS', 'APOLLOHOSP.NS', 'NESTLEIND.NS', 'SAREGAMA.NS'],
    index=0  # Set the default index to 0 to select the first option by default
)
